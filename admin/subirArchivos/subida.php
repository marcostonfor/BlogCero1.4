<?php
session_start(); // Iniciar sesión para usar los mensajes flash
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_FILES['fichero_usuario'])) {
    ...
    if (empty($archivos['name'][0]) && $archivos['error'][0] == UPLOAD_ERR_NO_FILE) {
        redirigirConMensaje("⚠️ No se seleccionó ningún archivo para subir. Por favor, seleccione al menos un archivo .md.");
    }
    ...
    for ($i = 0; $i < $num_archivos_seleccionados; $i++) {
        ...
        if ($extension !== 'md') {
            redirigirConMensaje("❌ Solo se permiten archivos .md. El archivo '" . htmlspecialchars($nombre_archivo_original) . "' no es válido.");
        }
